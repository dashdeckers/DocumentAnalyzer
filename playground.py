...
class SetCategoryNames(tk.Toplevel):
    def __init__(self, master, n_cats):
        super().__init__()
        self.master = master
...
class Notebook(tk.Tk):
    def __init__(self):
        ...
        self.cat_names = list(wordlists.keys())     # this stuff needs to happen in a function after creating or opening a project
        cat_buttons = list()
        for i in range(len(self.cat_names)):
            cat_buttons.append(ttk.Button(self.buttons_frame, text=self.cat_names[i] + f' ({i})', command=lambda event=0, b_num=i, b_name=self.cat_names[i]: self.dummy_function(event, b_num, b_name)))
    ...
    def dummy_function(self, event, cat_num, cat_name):
...
#        print('nope')
    ...
    def set_bindings(self, event=None):
        if self.notebook.tab(self.notebook.select(), 'text') == 'Classify':
            for i in range(len(self.cat_names)):
                self.bind(str(i), lambda event=0, b_num=i, b_name=self.cat_names[i]: self.dummy_function(event, b_num, b_name))
        else:
            for i in range(len(self.cat_names)):
                self.unbind(str(i))
    ...
    def spellcheck(self, event=None):
        ...
        words = text.split()
        spelling_errors = self.spell.unknown(words)
